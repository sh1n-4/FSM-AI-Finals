...
namespace Platformer {
    public class PlayerController : ValidatedMonoBehaviour {
        ...
        public void Attack() {
            Vector3 attackPos = transform.position + transform.forward * (attackDistance * 0.5f) + Vector3.up * 1f;
            Collider[] hitEnemies = Physics.OverlapSphere(attackPos, attackDistance);
            Debug.DrawRay(transform.position, transform.forward * attackDistance, Color.red, 1f);

            foreach (var enemy in hitEnemies) {
                Debug.Log(enemy.name);
                if (enemy.CompareTag("Enemy")) {
                    enemy.GetComponentInParent<Health>().TakeDamage(attackDamage);
                    Debug.Log($"Hit object: {enemy.name}, Tag: {enemy.tag}");
                }
            }
        }
